#!/usr/bin/env python3
"""
Ground Truth (Claude Opus) vs Gemini Comparison Analysis
...
def load_gemini_results(filepath):
    """Load Gemini results."""
    results = {}
    with open(filepath, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
...
def compare_query(gt_data, gemini_data, query):
    """
    Compare Ground Truth (Claude) vs Gemini for a single query.
    Returns detailed comparison metrics with priorities.
    """
    result = {
        'query': query,
        'gt_success': gt_data.get('success', False) if gt_data else False,
        'gemini_success': gemini_data.get('success', False) if gemini_data else False,
        'gt_total_keys': gt_data.get('total_keys', 0) if gt_data else 0,
        'gt_unique_values': gt_data.get('unique_values', 0) if gt_data else 0,
        'gemini_total_keys': gemini_data.get('total_keys', 0) if gemini_data else 0,
        'gemini_unique_values': gemini_data.get('unique_values', 0) if gemini_data else 0,
        'matched_keys': [],
        'matched_values': [],
        'gt_only_keys': [],
        'gemini_only_keys': [],
    ...
    if not gt_data or not gemini_data:
        return result
    
    gt_attrs = gt_data.get('attributes', [])
    gemini_attrs = gemini_data.get('attributes', [])
    ...
    # Create lookup for Gemini
    gemini_keys_normalized = {}  # normalized_key -> list of entries
    gemini_values_normalized = {}  # normalized_value -> list of entries
    
    for attr in gemini_attrs:
        key = attr.get('attribute_key', '')
        value = attr.get('value', '')
        key_priority = attr.get('key_priority', 0)
        attr_priority = attr.get('attribute_priority', 0)
        ...
        if norm_key not in gemini_keys_normalized:
            gemini_keys_normalized[norm_key] = []
        gemini_keys_normalized[norm_key].append({
            'original_key': key,
            'value': value,
            'norm_value': norm_value,
            'key_priority': key_priority,
        ...
        if norm_value not in gemini_values_normalized:
            gemini_values_normalized[norm_value] = []
        gemini_values_normalized[norm_value].append({
            'original_value': value,
            'key': key,
            'norm_key': norm_key,
            'key_priority': key_priority,
    ...
    # Find matched keys
    matched_keys = set(gt_keys_normalized.keys()) & set(gemini_keys_normalized.keys())
    gt_only = set(gt_keys_normalized.keys()) - set(gemini_keys_normalized.keys())
    gemini_only = set(gemini_keys_normalized.keys()) - set(gt_keys_normalized.keys())
    
    # Find matched values
    matched_values = set(gt_values_normalized.keys()) & set(gemini_values_normalized.keys())
    
    result['matched_keys'] = list(matched_keys)
    result['matched_values'] = list(matched_values)
    result['gt_only_keys'] = list(gt_only)
    result['gemini_only_keys'] = list(gemini_only)
    
    # Detailed key match info with priorities
    key_match_details = []
    for norm_key in matched_keys:
        gt_entries = gt_keys_normalized[norm_key]
        gemini_entries = gemini_keys_normalized[norm_key]
        
        # Find matches by value within this key
        for gt_entry in gt_entries:
            for gemini_entry in gemini_entries:
                value_match = gt_entry['norm_value'] == gemini_entry['norm_value']
                ...
                key_match_details.append({
                    ...
                    'gemini_key': gemini_entry['original_key'],
                    'gemini_value': gemini_entry['value'],
                    'gemini_key_priority': gemini_entry['key_priority'],
                    'gemini_attr_priority': gemini_entry['attribute_priority'],
                    'value_also_matches': value_match,
                    'key_priority_match': gt_entry['key_priority'] == gemini_entry['key_priority'],
                    'attr_priority_match': gt_entry['attribute_priority'] == gemini_entry['attribute_priority']
                })
    ...
    for norm_val in matched_values:
        ...
        gemini_entries = gemini_values_normalized[norm_val]
        
        # Find if they're under the same key
        gt_keys = set(e['norm_key'] for e in gt_entries)
        gemini_keys = set(e['norm_key'] for e in gemini_entries)
        same_key = bool(gt_keys & gemini_keys)
        
        value_match_details.append({
            'value': norm_val,
            'gt_original': gt_entries[0]['original_value'] if gt_entries else '',
            'gemini_original': gemini_entries[0]['original_value'] if gemini_entries else '',
            'gt_keys': [{'key': e['key'], 'key_priority': e['key_priority'], 'attr_priority': e['attribute_priority']} for e in gt_entries],
            'gemini_keys': [{'key': e['key'], 'key_priority': e['key_priority'], 'attr_priority': e['attribute_priority']} for e in gemini_entries],
        ...
        if detail['value_also_matches']:
            if detail['key_priority_match'] and detail['attr_priority_match']:
                ...
                partial_matches.append({
                    ...
                    'gemini_priorities': f"key_prio={detail['gemini_key_priority']}, attr_prio={detail['gemini_attr_priority']}"
                })
...
def main():
    ...
    gemini_path = 'gemini_final_results.csv'
    output_path = '../present/gt_vs_gemini_detailed_comparison.csv'
    summary_path = '../present/gt_vs_gemini_summary.csv'
    ...
    gemini_results = load_gemini_results(gemini_path)
    ...
    all_queries = set(gt_results.keys()) | set(gemini_results.keys())
    
    for query in sorted(all_queries):
        gt_data = gt_results.get(query)
        gemini_data = gemini_results.get(query)
        
        comparison = compare_query(gt_data, gemini_data, query)
    ...
    with open(output_path, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        writer.writerow([
            ...
            'gt_unique_values',
            'gemini_total_keys',
            'gemini_unique_values',
            'matched_key_count',
            'matched_value_count',
            'gt_only_key_count',
            'gemini_only_key_count',
            'key_match_rate',
            'value_match_rate',
            'perfect_match_count',
            'partial_match_count',
        ...
        for comp in comparisons:
            ...
            gemini_unique_keys = len(set(normalize_key(a.get('attribute_key', '')) 
                                        for a in gemini_results.get(comp['query'], {}).get('attributes', [])))
            ...
            writer.writerow([
                ...
                comp['gemini_total_keys'],
                comp['gemini_unique_values'],
                len(comp['matched_keys']),
                len(comp['matched_values']),
                len(comp['gt_only_keys']),
                len(comp['gemini_only_keys']),
                f"{key_match_rate:.2%}",
    ...
    both_success = sum(1 for c in comparisons if c['gt_success'] and c['gemini_success'])
    gt_only_success = sum(1 for c in comparisons if c['gt_success'] and not c['gemini_success'])
    gemini_only_success = sum(1 for c in comparisons if not c['gt_success'] and c['gemini_success'])
    both_fail = sum(1 for c in comparisons if not c['gt_success'] and not c['gemini_success'])
    
    total_gt_keys = sum(c['gt_total_keys'] for c in comparisons)
    total_gemini_keys = sum(c['gemini_total_keys'] for c in comparisons)
    ...
    queries_with_perfect_key_match = sum(1 for c in comparisons 
                                         if c['gemini_total_keys'] > 0 and 
                                         len(c['gemini_only_keys']) == 0 and
    ...
    all_gemini_keys = defaultdict(int)
    ...
    for comp in comparisons:
        ...
        gemini_data = gemini_results.get(comp['query'], {})
        for attr in gemini_data.get('attributes', []):
            norm_key = normalize_key(attr.get('attribute_key', ''))
            all_gemini_keys[norm_key] += 1
    ...
    with open(summary_path, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        writer.writerow(['GROUND TRUTH (CLAUDE OPUS) VS GEMINI - COMPARISON SUMMARY'])
        writer.writerow([])
        ...
        writer.writerow(['Gemini Only Success', gemini_only_success])
        ...
        writer.writerow(['Total Gemini Keys (with synonyms)', total_gemini_keys])
        ...
        writer.writerow(['Key', 'Count', 'Also in Gemini?', 'Match Count'])
        for key, count in sorted(all_gt_keys.items(), key=lambda x: -x[1])[:30]:
            in_gemini = 'Yes' if key in all_gemini_keys else 'No'
            match_count = matched_key_freq.get(key, 0)
            writer.writerow([key, count, in_gemini, match_count])
        writer.writerow([])
        
        writer.writerow(['=== TOP 30 GEMINI KEYS (by frequency) ==='])
        writer.writerow(['Key', 'Count', 'Also in GT?', 'Match Count'])
        for key, count in sorted(all_gemini_keys.items(), key=lambda x: -x[1])[:30]:
        ...
        writer.writerow(['Key', 'Match Count', 'GT Frequency', 'Gemini Frequency'])
        for key, count in sorted(matched_key_freq.items(), key=lambda x: -x[1])[:30]:
            writer.writerow([key, count, all_gt_keys.get(key, 0), all_gemini_keys.get(key, 0)])
        writer.writerow([])
