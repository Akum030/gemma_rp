"""
Merge and format QMeans and Gemini Zero-Shot results into a single comparison file
...
def main():
    ...
    print("Loading Gemini Zero-Shot results...")
    gemini_df = pd.read_csv('gemini_zero_shot_validation_queries.csv')
    print(f"  Loaded {len(gemini_df)} rows")
    
    # Check column names in gemini file
    print(f"\nGemini columns: {gemini_df.columns.tolist()}")
    
    # Identify the correct column names for gemini zero-shot
    gemini_count_col = [c for c in gemini_df.columns if 'attr_count' in c.lower()][0]
    gemini_isq_col = [c for c in gemini_df.columns if 'isq_list' in c.lower()][0]
    
    print(f"Using Gemini columns: count={gemini_count_col}, isq={gemini_isq_col}")
    
    # Merge on query
    print("\nMerging dataframes...")
    merged_df = pd.merge(
        qmeans_df[['query', 'qmeans_attr_count', 'qmeans_isq_list']],
        gemini_df[['query', gemini_count_col, gemini_isq_col]],
        on='query',
        how='outer'
    )
    ...
    merged_df = merged_df.rename(columns={
        gemini_count_col: 'gemini_attr_count',
        gemini_isq_col: 'gemini_isq_list'
    })
    ...
    max_attrs = max(
        max(len(a) for a in qmeans_attrs) if qmeans_attrs else 0,
        max(len(a) for a in gemini_attrs) if gemini_attrs else 0
    )
    ...
    # Save to CSV
    output_file = 'formatted_comparison_zeroshot.csv'
    ...
    print(f"Gemini Zero-Shot avg attributes: {output_df['gemini_attr_count'].mean():.2f}")
