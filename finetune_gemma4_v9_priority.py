"""
Gemma-4 E4B fine-tune  V9 PRIORITY  (post-v8 accuracy push).

Goal:
  - Improve key_priority ranking and attribute values using the same nested schema
  - Keep the overnight runtime practical on a single T4
  - Increase the influence of the human-labeled gold rows without exploding runtime

Differences vs V8:
  - Fresh run from the base Gemma-4 E4B model
  - Same LoRA capacity as V8 (r=64, alpha=128)
  - Lower LR for cleaner refinement: 5e-5
  - 3 epochs
  - Gold examples oversampled 3x in train split
  - Slightly smaller eval/save cadence to monitor quality sooner

Note:
  - This remains a deployable model, not a clean benchmark, because gold_1k_v2 is used
    for training. The publishable clean baseline is still v7-vs-gold.
"""
...
OUTPUT_DIR = "/home3/indiamart/gemma_4/isq-gemma4-e4b-v9-priority"
...
V9_TRAIN_OUT = "/home3/indiamart/gemma_4/v9_train.jsonl"
V9_VAL_OUT   = "/home3/indiamart/gemma_4/v9_val.jsonl"
...
def flat_to_nested(flat):
    ...
    nested = []
    for i, (_rank, _key, value, synonyms) in enumerate(items, start=1):
        obj = {"value": value}
...
def main():
    print("=" * 70)
    print("GEMMA-4 E4B  V9 PRIORITY  accuracy push")
    print("=" * 70)
    random.seed(42)

    print("\n[1/5] Building V9 dataset ...")
    ...
    all_train = cat_train + v6_train + boosted_gold_train
    ...
    print(f"   gold boosted x{GOLD_TRAIN_REPEAT}: train={len(boosted_gold_train)}")
    print(f"   COMBINED:          train={len(all_train)} val={len(all_val)}")

    write_jsonl(all_train, V9_TRAIN_OUT)
    write_jsonl(all_val, V9_VAL_OUT)
