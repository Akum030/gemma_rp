"""
Gemma-4 E4B fine-tune  V12 PRIORITY RANK FOCUS.

Goal:
  - continue from V11 stability
  - improve attribute priority ordering and key-priority ordering
  - avoid V8-V10 style recall collapse

Design hypothesis:
  - V11 already balances clean structure and broad coverage
  - ranking quality should improve if the model sees more multi-attribute rows
  - oversampling rows with 2+ and 3+ attributes is safer than aggressive clean-only shifts

Data recipe:
  1. cat74 nested priority rows (lightly repeated)
  2. v6 flat->nested rows (broad coverage)
  3. gold_1k_v2 flat rows converted to nested (light repeat)
  4. rank-focused oversampling for rows with >=2 and >=3 attributes
"""
import os
import sys
import json
...
OUTPUT_DIR = "/home3/indiamart/gemma_4/isq-gemma4-e4b-v12-priority-rankfocus"
...
V12_TRAIN_OUT = "/home3/indiamart/gemma_4/v12_train.jsonl"
V12_VAL_OUT = "/home3/indiamart/gemma_4/v12_val.jsonl"
...
def attr_count_from_output(output):
    try:
...
def main():
    print("=" * 70)
    print("GEMMA-4 E4B  V12 PRIORITY RANK FOCUS")
    print("=" * 70)
    random.seed(42)

    print("\n[1/5] Building V12 dataset ...")
    ...
    base_train = boosted_cat_train + v6_train + boosted_gold_train

    multi_attr_rows = [row for row in base_train if attr_count_from_output(row["output"]) >= 2]
    high_multi_rows = [row for row in base_train if attr_count_from_output(row["output"]) >= 3]

    rank_focus_train = base_train + (multi_attr_rows * MULTI_ATTR_REPEAT) + (high_multi_rows * HIGH_MULTI_REPEAT)
    all_val = cat_val + v6_val + gold_val

    random.shuffle(rank_focus_train)
    ...
    print(f"   base train before rankfocus: {len(base_train)}")
    print(f"   multi-attr (>=2) rows:      {len(multi_attr_rows)} x{MULTI_ATTR_REPEAT}")
    print(f"   high-multi (>=3) rows:      {len(high_multi_rows)} x{HIGH_MULTI_REPEAT}")
    print(f"   COMBINED rankfocus train:   {len(rank_focus_train)}")
    print(f"   COMBINED val:               {len(all_val)}")

    write_jsonl(rank_focus_train, V12_TRAIN_OUT)
    write_jsonl(all_val, V12_VAL_OUT)
    ...
    print("\n[4/5] Tokenising ...")
    train_ds = to_text_ds(rank_focus_train)
